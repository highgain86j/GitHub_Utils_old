...
while getopts hdi:p:r:u: OPT
	do
	case $OPT in
		"h" ) FLG_H="TRUE" ;;
		"d" ) FLG_D="TRUE" ;;
		"i" ) FLG_I="TRUE" ; user_incl="$OPTARG" ;;
		"p" ) FLG_P="TRUE" ; auth_pswd="$OPTARG" ;;
		"r" ) FLG_R="TRUE" ; repo_path="$OPTARG" ;;
		"u" ) FLG_U="TRUE" ; auth_user="$OPTARG" ;;
		* ) FLG_H="TRUE" ;;
	esac
done
...
function usage {
	echo "Usage:"
	echo "./AutoSyncFork.sh -h"
	echo "./AutoSyncFork.sh -u <USERNAME> -p <PASSWORD> [-i <EXTERNAL ACCOUNTS>] [-d] [-r <CLONE DIRECTORY>]"
	exit 0
}
...
if [ "$FLG_I" = "TRUE" ]; then
	echo "External accounts specified as:"
	echo ${user_incl}
fi
...
showhelp=$(($user_unspec | $pswd_unspec))
if [ "${showhelp}" = 1 ]; then
	usage
fi
...
acnt_prvt=${auth_user}
#user_incl="kicad-jp"
user_incl="kicad-jp KiCad"

acnt_ofcl="${acnt_prvt} ${user_incl}"
#acnt_ofcl="${user_incl}"
...
addpendf=${work_dir}/${penddir}/fork_pending_${addflg}.${repofext}
delpendf=${work_dir}/${penddir}/fork_pending_${delflg}.${repofext}
		...
		if [ ${opmode} = add ]
			then
				...
				for source in `cat ${tmp_0}`
					...
					echo "> Found;"
					echo ${source}
					echo "> "${sourceowner}"/"${sourcerepos}" successfully forked to "`curl -u ${auth_info} -X POST https://api.github.com/repos/${sourceowner}/${sourcerepos}/forks 2> /dev/null | jq '.full_name' | sed -e s/\"//g`
				done
				echo ""
		elif [ ${opmode} = del ]
			then
				...
				for deltar in `cat ${tmp_0}`
					...
					curl -X DELETE -u ${auth_info} https://api.github.com/repos/${acnt_prvt}/${deltarrepos} 2> /dev/null
				done
				echo ""
		else
			echo ""
...
function reposprobe {
	for acnt in ${acnt_ofcl} 
		...
		curl -u ${auth_info} https://api.github.com/${acntt}/${acntn}/repos?per_page=100 2> /dev/null | jq '.[].name' > ${tmp_0}
		sed -i -e 's/"//g' ${tmp_0}

		if [ ${acntn} != ${acnt_prvt} ] ;
			then

				reposdir=${work_dir}/${acntn}.${sffxdir}
		...
		for repos in `cat ${tmp_0}`
			...
			reposinfo=`curl -u ${auth_info} -G https://api.github.com/repos/${acntn}/${repos} 2> /dev/null \
				...
				then
					...
					if [ ${reposinf_1} = true ] ;
						then
							...
							reposlist=${work_dir}/${reposfdir}/${reposinf_6}_${sffxfork}
							reposstrg=${reposinf_6},${reposinf_2},${reposinf_7}

						else

							reposlist=${work_dir}/${reposprv}
		...
		if [ ${acntn} != ${acnt_prvt} ] ;
			then
				...
				if [ ! -e ${work_dir}/${acntn}.${sffxdir}/${acntn}_${sffxfork} ] ;
					then 
						touch ${work_dir}/${acntn}.${sffxdir}/${acntn}_${sffxfork}
				fi

				for reposfil in ${work_dir}/${acntn}.${sffxdir}/${acntn}_${sffxsrc} ${work_dir}/${acntn}.${sffxdir}/${acntn}_${sffxfork}
					do
				...
				for opmode in add del
					...
					if [ ${opmode} = add ]
						then
							...
							diff ${work_dir}/${acntn}.${sffxdir}/${acntn}_${sffxsrc} ${work_dir}/${acntn}.${sffxdir}/${acntn}_${sffxfork} | grep '^<' \
							| sed -e "s/< //g" > ${infile}
...
function gitclone {
	...
	if [ ! -e ${repo_dir} ]
		then
			echo `echo_yellow ${repo_dir}`" does not exist. Creating now."
			mkdir ${repo_dir}
	fi
	for clonelst in `ls ${work_dir}/*.${sffxdir}/*_${sffxfork}` `ls ${work_dir}/*.${sffxdir}/*_${sffxprvt}`
		do
	...
	for clonelne in `cat ${listall}`
		...
		git_url1="git@github.com:${auth_user}/${var_repname}.git"
		git_url2="${var_git_url}"
		gitdir2=${repo_dir}/${var_ownerid}
		if [ ! -e ${gitdir2} ]
			then
				mkdir ${gitdir2}
		fi
...
function repoprocess {
	...
	for opmode in add del
		...
		for pendingf in `ls ${work_dir}/*.${sffxdir}/*_${suffix}`
			do
...
function cleanupf {
	...
	for garbage in `ls ${work_dir}/*.${sffxdir}/*.${repofext}` ${tmp_0} ${tmp_1} ${tmp_2} ${tmp_3} ${tmp_4} ${tmp_5} ${tmp_6} ${tmp_7} ${tmp_8} ${tmp_9}
		do
		echo "   Deleting;"
		echo_yellow "      "${garbage}
		rm ${garbage}
